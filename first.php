<!DOCTYPE html>
<html>
    <head>
        <title>Diwali</title>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <link rel="stylesheet" href="./assets/css/bootstrap.min.css">
        <script src="./assets/js/jquery.min.js"></script>
        <link rel="stylesheet" type="text/css" href="./assets/css/style.css">
        <link href="https://fonts.googleapis.com/css?family=Barlow+Condensed:200i,300i,400,500,600,900" rel="stylesheet">
        <link href="https://fonts.googleapis.com/css?family=Montserrat:300,400,700,900" rel="stylesheet">
        <link rel="stylesheet" type="text/css" href="./assets/css/aos.css">
        <script type="text/javascript" src="./assets/js/aos.js"></script>
    </head>
    <body>
        <div class="header">
             <div class="right-cart" style="position: relative;"><img src="./assets/img/cart.png">
             <p class="topleft" style="margin-top: -50px;  font-size: 16px;font-weight: 600; color: yellow;">5</p>
             </div>
        </div>
        
        <div class="header-top">
            <p class="left-lamp"><img src="./assets/img/L.png"></p>
            <p class="right-lamp"><img src="./assets/img/R.png"></p>
            <div class="header-container">
                <p class="title"><img src="./assets/img/Sale.png"/></p>
                <p class="up-to">-UPTO-</p>
                <p class="percent">90% off</p>
                <p class="sale-desc">Get flat 90% off on all transformation systems! Choose the product of your choice and avail this exclusive, limited period offer. Transform your life, make it even better. Take action now!
                    Offer available for limited time only
                </p>
                <p class="note">offer available for limited time only</p>
            </div>
        </div>
        <div class="body-container">
        <div class="counter">
            <div id="js-clock" class="clock">
                <div class="clock__days">
                    <span id="js-days"></span>
                    <div>Days</div>
                </div>
                <div class="clock__hours">
                    <span id="js-hours"></span>
                    <div>Hours</div>
                </div>
                <div class="clock__minutes">
                    <span id="js-minutes"></span>
                    <div>Minutes</div>
                </div>
                <div class="clock__seconds">
                    <span id="js-seconds"></span>
                    <div>Seconds</div>
                </div>
            </div>
        </div>
        <!------------------------------------------------SMB PRODUCT------------------------------------------------>
        <div class="body-inner">
            <div class="offer-div" style="padding: 20px 40px 20px 40px;">
                <p class="title"><span>The Secret Wealth Formula</span></p>
                <p class="sub-title">Discover the money blueprint of millionaires <br/>and create a legacy</p>
                <p class="offer-price">Rs.82,997</p>
            </div>
            <div class="event-div event-blueprint" >
                <div class="event-info" style="width:85%">
                    <p class="event-title1">The Secret Millionaire Blueprint <span>Two day live event</span></p>
                    <p class="event-subtitle1">RS.25,000</p>
                    <p class="event-subtitle1">+</p>
                    <p class="event-title1">The Secret Wealth Formula Fast Start Program <span>Exclusive videos to get you started immediately</span></p>
                    <p class="event-subtitle1">RS.39,997</p>
                    <p class="event-subtitle1">+</p>
                    <p class="event-title1">The Mastermind Group Of Champions<span>Exclusive community</span></p>
                    <p class="event-subtitle1">RS.18,000</p>
                    <p class="know" id="know-smb">Know More</p>
                </div>
                <div class="footer-offer" style="margin-top:0px;">
                    <div class=""><span class="strike">Rs.82,997</span>  <span class="new-vaule">Rs.8300 Only</span> </div>
                    <div class="">     <a class="add-cart" href="#"><span class="orange">+</span> Click & add to cart</a></div>
                </div>
            </div>
        </div>
        <!-------------------------------------------SECOND-DIV(PRODUCT_DETAILS)------------------------------------------------------------------>

<div class="body-innersmb" style="display:none;">
            <div class="offer-div">
                <p class="title"><span>The Secret Wealth Formula</span></p>
                <p class="sub-title">Discover the money blueprint of millionaires <br/>and create a legacy</p>
                <p class="offer-price">Rs.82,997</p>
            </div>
            <div class="event-div event-blueprint">
                <div class="event-info">
                    <p class="event-title">The Secret Millionaire Blueprint <span>Two day live event</span></p>
                    <p class="event-subtitle">A Live Two Day Event, where I literally break down the
                        complete system to re-wire your financial blueprint and
                        your mindset so you can create wealth beyond your
                        imagination!
                    </p>
                    <div class="event-rule">
                        <ul data-aos="fade-down">
                            <li>Learn the law of attraction and why it may not be working and what to do about it.</li>
                            <li>Powerfully experience where you are now in your life, envision exactly where you want to be, and Learn how to close the gap between the two.</li>
                            <li>Attract abundance and success in all areas of your life.</li>
                            <li>The Secret Psychology of Wealth.</li>
                            <li>Master the powerful skills of rapport and influence to maximize your effectiveness as a parent, partner, business person, and leader.</li>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="event-div  right-side event-flex no-bg-event ">
                <img src="./assets/img/iMac.png" class="event-poster" data-aos="fade-right">
                <div class="event-info">
                    <p class="event-title">The Secret Wealth Formula Fast Start Program<span>Exclusive videos to get you started immediately</span></p>
                    <p class="event-subtitle">Get access to a series of exclusive videos, total of 5 hours, where you
                        will discover the strategies and habits of every wealthy person on
                        the planet which are overlooked by most of the population in the
                        world. Get started on...
                    </p>
                    <div class="event-rule">
                        <ul>
                            <li>The money blueprint for abundance of wealth</li>
                            <li>The engines that drive all millionaires</li>
                            <li>The secret formula to attract wealth</li>
                            <li>And much, much more!</li>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="event-div no-bg-event event-flex mast-event">
                <div class="event-info">
                    <p class="event-title">The Mastermind Group of Champions<span>Exclusive Community</span></p>
                    <p class="event-subtitle">Be part of the exclusive mastermind group where you will meet awesome,
                        amazing people who are driven to help others, are go getters! Take your life
                        to a new level just by being part of this community!
                    </p>
                </div>
                <div class="event-poster">
                    <img src="./assets/img/iPad Air Space Gray.png" data-aos="fade-right">
                </div>
            </div>
            <div class="footer-offer">
                <div class=""><span class="strike">Rs.82,997</span>  <span class="new-vaule">Rs.8300 Only</span> </div>
                <div class="">     <a class="add-cart" href="#"><span class="orange">+</span> Click & add to cart</a></div>
            </div>
        </div>
        <!-------------------------------------STF PRODUCT------------------------------>
        
        <div class="body-inner12" style="margin-top: 40px;">
            <div class="offer-div"  style="padding: 20px 40px 20px 40px;">
                <p class="title"><span>Speak to a Fortune System</span></p>
                <p class="sub-title">Discover the money blueprint of millionaires <br/>and create a legacy</p>
                <p class="offer-price">Rs.82,997</p>
            </div>
            <div class="event-div event-blueprint" >
                <div class="event-info" style="width:85%">
                    <p class="event-title1">The Secret Millionaire Blueprint <span>Two day live event</span></p>
                    <p class="event-subtitle1">RS.25,000</p>
                    <p class="event-subtitle1">+</p>
                    <p class="event-title1">STF Fast Start Program <span>Private, closed - door recording</span></p>
                    <p class="event-subtitle1">RS.39,997</p>
                    <p class="event-subtitle1">+</p>
                    <p class="event-title1">The Mastermind Group Of Champions<span>Exclusive community</span></p>
                    <p class="event-subtitle1">RS.18,000</p>
                    <p class="know" id="know-stf">Know More</p>
                </div>
                <div class="footer-offer" style="margin-top:0px;">
                    <div class=""><span class="strike">Rs.82,997</span>  <span class="new-vaule">Rs.8300 Only</span> </div>
                    <div class=""><a class="add-cart" href="#"><span class="orange">+</span> Click & add to cart</a></div>
                </div>
            </div>
        </div>
        
        <!-------------------------------------------SECOND-DIV(PRODUCT_DETAILS)------------------------------------------------------------------>

<div class="body-innerstf" style="display:none;">
            <div class="offer-div">
                <p class="title"><span>Speak to a Fortune System</span></p>
                <p class="sub-title">Discover the money blueprint of millionaires <br/>and create a legacy</p>
                <p class="offer-price">Rs.82,997</p>
            </div>
            <div class="event-div event-blueprint">
                <div class="event-info">
                    <p class="event-title">The Secret Millionaire Blueprint <span>Two day live event</span></p>
                    <p class="event-subtitle">A Live Two Day Event, where I literally break down the
                        complete system to re-wire your financial blueprint and
                        your mindset so you can create wealth beyond your
                        imagination!
                    </p>
                    <div class="event-rule">
                        <ul data-aos="fade-down">
                            <li>Learn the law of attraction and why it may not be working and what to do about it.</li>
                            <li>Powerfully experience where you are now in your life, envision exactly where you want to be, and Learn how to close the gap between the two.</li>
                            <li>Attract abundance and success in all areas of your life.</li>
                            <li>The Secret Psychology of Wealth.</li>
                            <li>Master the powerful skills of rapport and influence to maximize your effectiveness as a parent, partner, business person, and leader.</li>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="event-div  right-side event-flex no-bg-event ">
                <img src="./assets/img/STF.png" class="event-poster" data-aos="fade-right" style="width: 40%;">
                <div class="event-info">
                    <p class="event-title">STF Fast Start Program<span>Private, closed - door recording</span></p>
                    <p class="event-subtitle">The outcome here is that you start taking action without wasting any time! Go through the video based training to start your action course on..
                    </p>
                    <div class="event-rule">
                        <ul>
                            <li>Creating your identity</li>
                            <li>Creating transformational content</li>
                            <li>Generating unlimited leads online</li>
                            <li>Ace Speaker Hacks</li>
                            <li>Make a fortune</li>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="event-div no-bg-event event-flex mast-event">
                <div class="event-info">
                    <p class="event-title">The Mastermind Group of Champions<span>Exclusive Community</span></p>
                    <p class="event-subtitle">Be part of the exclusive mastermind group where you will meet awesome,
                        amazing people who are driven to help others, are go getters! Take your life
                        to a new level just by being part of this community!
                    </p>
                </div>
                <div class="event-poster">
                    <img src="./assets/img/iPad Air Space Gray.png" data-aos="fade-right">
                </div>
            </div>
            <div class="footer-offer">
                <div class=""><span class="strike">Rs.82,997</span>  <span class="new-vaule">Rs.8300 Only</span> </div>
                <div class="">     <a class="add-cart" href="#"><span class="orange">+</span> Click & add to cart</a></div>
            </div>
        </div>
        
        
        
        
        
        
        
        
        
        </div>







        
<script>
$(document).ready(function(){
    $("#know-smb").click(function(){
        // alert("The paragraph was clicked.");
    // $(".body-innersmb").css("display","block");
    $(".body-inner").css("display","none");
      $(".body-innersmb").css("display","block");
    });
    $("#know-stf").click(function(){
        // alert("The paragraph was clicked.");
    // $(".body-innersmb").css("display","block");
    $(".body-inner12").css("display","none");
      $(".body-innerstf").css("display","block");
    });
});
</script>
        
        
        
         <script type="text/javascript">
            function countdown(endDate) {
             let days, hours, minutes, seconds;
             
             endDate = new Date(endDate).getTime();
             
             if (isNaN(endDate)) {
            return;
             }
             
             setInterval(calculate, 1000);
             
             function calculate() {
               let startDate = new Date();
               startDate = startDate.getTime();
               
               let timeRemaining = parseInt((endDate - startDate) / 1000);
               
               if (timeRemaining >= 0) {
                 days = parseInt(timeRemaining / 86400);
                 timeRemaining = (timeRemaining % 86400);
                 
                 hours = parseInt(timeRemaining / 3600);
                 timeRemaining = (timeRemaining % 3600);
                 
                 minutes = parseInt(timeRemaining / 60);
                 timeRemaining = (timeRemaining % 60);
                 
                 seconds = parseInt(timeRemaining);
                 
                 document.getElementById("js-days").innerHTML = parseInt(days, 10);
                 document.getElementById("js-hours").innerHTML = ("0" + hours).slice(-2);
                 document.getElementById("js-minutes").innerHTML = ("0" + minutes).slice(-2);
                 document.getElementById("js-seconds").innerHTML = ("0" + seconds).slice(-2);
               } else {
                 return;
               }
             }
            }
            
            (function () { 
             countdown(new Date().getFullYear() + 1 + '-01-09'); 
            }());
        </script>
        <script>
            AOS.init({
            	easing: 'ease-out-back',
            	duration: 1000
            });
        </script>
        
    </body>
    </html>